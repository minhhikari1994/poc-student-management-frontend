        ...
        <div class="relative z-[2] w-full max-w-[340px] px-5">
            ...
            <form class="mt-10" @submit="submit">
                <fieldset :disabled="isSubmitting" class="grid gap-5">
                    ...
                    <UiVeeInput label="Xác nhận mật khẩu" type="password" name="confirmPassword"
                        placeholder="" />
                    <ul class="flex flex-col gap-4">
                        <li class="flex items-center gap-3 text-sm text-muted-foreground">
                            <Icon :class="[meta.valid ? 'text-green-500' : '']" class="size-4"
                        ...
                        <li class="flex items-center gap-3 text-sm text-muted-foreground">
                            <Icon :class="[meta.valid ? 'text-green-500' : '']" class="size-4"
                                name="lucide:check-circle-2" />
                            <span>Chứa ít nhất 1 kí tự số & 1 kí tự đặc biệt</span>
                        </li>
                    </ul>
...
import { useUserAccountStore } from "@/stores/userStore";
...
const { handleSubmit, isSubmitting, meta, resetForm } = useForm<InferType<typeof LoginSchema>>({
    validationSchema: LoginSchema,
});

const submit = handleSubmit(async (formData) => {
    const changePasswordResult = await userAccountStore.changePassword(
